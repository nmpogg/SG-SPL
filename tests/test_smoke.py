...
def run():
    ...
    from src.losses import (
        ...
        structural_losses, prototype_contrastive_loss
    )
    ...
    # L_proto
    loss_proto = prototype_contrastive_loss(sk_f, ph_f, cidx, bank2, warmup=3)
    print(f'   L_proto = {loss_proto.item():.4f}')

    # Check no NaN
    for name, val in [('cls', loss_cls), ('ssc', loss_ssc), ('xmod', loss_xmod), ('proto', loss_proto)]:
    ...
    total_loss = loss_cls + loss_ssc + loss_xmod + loss_proto
    total_loss.backward()
