...
# Optimiser
# ─────────────────────────────────────────────
parser.add_argument('--lr_prompt',  type=float, default=1e-4,
                    help='Learning rate for prompt parameters')
parser.add_argument('--lr_ln',      type=float, default=1e-6,
                    help='Learning rate for CLIP LayerNorm parameters')
...
# L_cls — classification loss (CLIP-AT baseline)
parser.add_argument('--l_cls',      type=float, default=1.0,
                    help='Weight for classification loss L_cls')

# L_SSC — Semantic Structure Consistency (Giai đoạn 1)
parser.add_argument('--l_ssc',      type=float, default=1.0,
                    help='Weight for L_SSC (set 0 to disable)')
...
parser.add_argument('--ssc_temp',   type=float, default=0.1,
                    help='Temperature T for KL variant of L_SSC')

# L_xmod — Cross-modal Structure Consistency (Giai đoạn 1)
parser.add_argument('--l_x',        type=float, default=0.5,
                    help='Weight for L_xmod inside L_SSC term (set 0 to ablate xmod)')

# L_asym_sph — Asymmetric Hyperspherical Anchoring (Giai đoạn 2)
...
parser.add_argument('--text_templates', type=str, nargs='+',
                    default=['a photo of a {}.', 'a sketch of a {}.',
                             'a drawing of a {}.', 'an image of a {}.'],
